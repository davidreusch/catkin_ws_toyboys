...
def closest_point(spline, targetpoint):
    ...
    while (step > 0 and low != high):
        arcsample = arc[low:high:step]
        xcoords = xspline(arcsample)
        ycoords = yspline(arcsample)

        # add the original indices to the points, so that we can find the 2 adjacent points in the original arc
        points = list((zip(range(low, high, step), xcoords, ycoords)))
    ...
    return Point(x=point[1], y=point[2], z=0.0)
...
spline_msg.scale.x = 2
spline_msg.scale.y = 2
spline_msg.scale.z = 2
...
while not rospy.is_shutdown():
    ...
    point = closest_point(spline, (clickedPoint.x, clickedPoint.y))
    click_msg = Marker()
    click_msg.type = 2
    ...
    click_msg.scale.x = 0.3
    click_msg.scale.y = 0.3
    click_msg.scale.z = 0.3

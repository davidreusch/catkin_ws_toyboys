...
def ransac(img):
    ...
    #oberen und unteren Teil auf null setzen
    rows, cols = thresh.shape
    thresh[0:220,:] = 0
    thresh[360:rows,:] = 0
    cv.imshow('afterThresh',thresh)

    #weisse Pixel extrahieren
    whitepixels = []
    for x in range(cols):
        for y in range(rows):
            if thresh[y,x] > 0:
	...
	print(int(xa), int(ya), int(xb), int(yb) ,"bla")
	cv.line(img, (int(xa),int(ya)), (int(xb),int(yb)), (0,255,0), 5)
        #cv.line(img, (int(yforxzero),0), (0,int(xforyzero)), (0,255,0), 5)
